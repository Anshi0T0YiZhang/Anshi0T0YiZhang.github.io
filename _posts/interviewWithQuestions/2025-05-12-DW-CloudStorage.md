...
### 15.Go语言中，切片里有几种属性体现其数据结构？

- 指针：指向切片底层数组的第一个元素
- 长度：表示切片中元素的数量
- 容量：表示切片底层数组的大小，即从切片的第一个元素到其底层数组末尾的元素数量

### 16.Go语言中， Channel被关闭了，再读写会发生什么？

- 读操作：当从已关闭的```channel```读取数据时，会立即返回该```channel```元素类型的零值，且```ok```标志位为```false```。例如，若```channel```是```int```类型，读操作会返回```0```和```false```。
- 写操作：向已关闭的```channel```写入数据会导致```panic```异常，程序会崩溃。这是因为关闭```channel```后再写入数据会破坏```channel```的状态，引发不可预期的结果。

### 17.Go语言中，会有一个什么语法来防止我程序挂掉？

在 Go 语言中，可以使用```recover```函数来防止程序因```panic```而挂掉。```recover```只能在```defer```函数中使用，用于捕获```panic```并恢复程序的正常执行。
...
func main() {
    defer func() {
        if r := recover(); r != nil {
            // 这里可以记录错误信息等
            fmt.Println("程序出现panic，已恢复:", r)
        }
    }()

    // 假设这里有一个可能会导致panic的操作，比如向已关闭的channel写数据
    ch := make(chan int, 1)
    close(ch)
    ch <- 1 // 这会导致panic
}
```
上述代码中，defer语句中的recover函数会捕获到panic，并打印出相关信息，使程序不会直接挂掉。这样就可以在程序出现异常时进行一些善后处理，而不是让程序崩溃。
